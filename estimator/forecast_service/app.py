...
db = InfluxDB('localhost', 8086, 'predictive-maintenance', 'turbofan')
...
@app.route('/turbofan/<device>', methods=['GET', 'POST'])
def turbofan(device):
  device = int(device)
  sequence_length = estimator.get_sequence_length()
  data = db.get_last(sequence_length, device, order=sequence_cols)
